...
const fragmentShader = `
  ...
  uniform sampler2D u_texture;
    ...
    void main() {
        ...
        vec2 mouseDirection = u_mouse - prevMouse;
...
function createTextTexture(text, font, size, color, fontWeight = "100") {
  ...
  ctx.font = `${fontWeight} ${fontSize}px "${font || "mona-sans"}`;
...
initializeScene(
  createTextTexture("Hello", "mona-sans", null, "#ffffff", "100")
);

function animateScene() {
  requestAnimationFrame(animateScene);

  mousePosition.x += (mousePosition.x - mousePosition.x) * easeFactor;
  mousePosition.y += (mousePosition.y - mousePosition.y) * easeFactor;
